...
const ClassBoard = () => {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [{ data: classInfo }, { data: obs }, { data: results }] = await Promise.all([
        supabase.from('classes').select('name, subject').eq('id', classId).single(),
        supabase
          .from('weekly_observations')
          .select('id, student_id, student_name, activity_name, content, feeling, created_at, week_number')
          .eq('class_id', classId)
          .eq('status', 'approved')
          .order('created_at', { ascending: false }),
        supabase
          .from('activity_results')
          .select('id, student_id, student_name, week_number, title, text_content, image_url, file_url, display_name, link_url, created_at')
          .eq('class_id', classId)
          .eq('status', 'approved')
          .order('created_at', { ascending: false }),
      ]);
